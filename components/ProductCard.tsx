...
export default function ProductCard({ product, sessionId, messageId, onFeedbackSubmit }: ProductCardProps) {
    ...
    return (
        <div className="glass-card rounded-xl overflow-hidden group relative flex flex-col h-full">
            <div className="relative aspect-square overflow-hidden">
                <img
                    src={product.image_url}
                    alt={product.title}
                    className={`object-cover w-full h-full transition-transform duration-500 group-hover:scale-110 ${showDescription ? 'scale-110 blur-sm' : ''}`}
                />

                {showDescription && (
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm p-4 overflow-y-auto animate-fade-in z-0">
                ...
                <div className="absolute top-2 right-2 z-10 flex gap-1">
                    <button
                        ...
                        className={`p-1.5 rounded-full backdrop-blur-md transition-colors ${showDescription ? 'bg-purple-600 text-white' : 'bg-black/50 text-white hover:bg-black/70'}`}
                    >
                        <Info size={16} />
                    </button>
