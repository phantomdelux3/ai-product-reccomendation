...
def load_models(device):
    ...
    # Use float32 for Florence-2 to avoid Windows CUDA compatibility issues
    florence_dtype = torch.float32
    print(f"      Using dtype: {florence_dtype} (for stability)")
...
def analyze_image_with_retry(model, processor, image, title, price, device, max_retries=MAX_RETRIES):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Ensure image is in RGB mode and has valid dimensions
            if image.mode != "RGB":
            ...
            if image.size[0] > max_dimension or image.size[1] > max_dimension:
                ...
                print(f"      ℹ Resized large image to {new_size[0]}x{new_size[1]} pixels")
            
            # Use float32 for stability on all platforms
            torch_dtype = torch.float32
            
            # Process inputs - this returns a BatchEncoding object
            # Use try-except to catch processor errors
            try:
                inputs = processor(text=prompt, images=image, return_tensors="pt")
            except Exception as proc_error:
                raise ValueError(f"Processor failed: {proc_error}")
            
            # Validate that inputs contains required keys
            if "input_ids" not in inputs or inputs["input_ids"] is None:
                raise ValueError("Processor did not return valid input_ids")
            
            if "pixel_values" not in inputs or inputs["pixel_values"] is None:
                raise ValueError("Processor did not return valid pixel_values")
            
            # Additional validation: check if pixel_values is a proper tensor
            pixel_vals = inputs["pixel_values"]
            if not isinstance(pixel_vals, torch.Tensor):
                raise ValueError(f"pixel_values is not a tensor, got {type(pixel_vals)}")
            
            # Check tensor shape exists and is valid
            try:
                pv_shape = pixel_vals.shape
                if pv_shape is None or len(pv_shape) == 0:
                    raise ValueError(f"pixel_values has invalid shape: {pv_shape}")
            except AttributeError as ae:
                raise ValueError(f"pixel_values tensor missing shape attribute: {ae}")
            
            # Manually move each tensor to device with correct dtype
            # Use explicit error handling for tensor operations
            try:
                input_ids = inputs["input_ids"].to(device)
            except Exception as e:
                raise ValueError(f"Failed to move input_ids to {device}: {e}")
            
            try:
                # First move to device, then convert dtype
                pixel_values = pixel_vals.to(device)
                if pixel_values.dtype != torch_dtype:
                    pixel_values = pixel_values.to(dtype=torch_dtype)
            except Exception as e:
                raise ValueError(f"Failed to move/convert pixel_values to {device} with dtype {torch_dtype}: {e}")
            ...
            with torch.no_grad():
                try:
                    ...
                    raise ValueError(f"Model generation failed: {gen_error}")
